...
struct MBMapView: UIViewRepresentable {
    ...
    @State var triggerRefreshView: Bool = false
    ...
    func makeUIView(context: Context) -> some UIView {
        ...
        let cameraOptions = CameraOptions(center: centerCoordinate, zoom: 12.4, bearing: -25, pitch: 0)
        ...
        // Create locatxion identifier icon "puck"
        ...
        mapView.mapboxMap.subscribe(context.coordinator, events: [MapEvents.cameraChanged])
...
//        }
    ...
    func updateUIView(_ uiView: UIViewType, context: Context) {
        if let mapView = uiView as? MapView {
            if $viewModel.hasLoadedMiras.wrappedValue == true {
                debugPrint("load")
                refreshMirasOnMap(mapView: mapView, context: context)
            }
            if $viewModel.hasLoadedMiras2.wrappedValue {
                refreshMirasOnMap(mapView: mapView, context: context)
                debugPrint("load2")
...
//        let miras = Mira.dummyMiras() //commented for dummy miras
        let zoom: Float = Float(mapView.cameraState.zoom)
        ...
        for mira in miras {
            ...
            try? mapView.viewAnnotations.add(annotationView(mira: mira, sourceLocation: userLocation, tag: i, zoomLevel: zoom, context: context), options: options)
            i += 1
        }
    ...
    private func annotationView(mira: Mira, sourceLocation: CLLocationCoordinate2D?, tag: Int, zoomLevel: Float, context: Context) -> UIView {
...
extension MBMapView {
    class MapViewCoordinator: NSObject, AnnotationInteractionDelegate, Observer {
        ...
        func notify(for event: Event) {
            ...
            DispatchQueue.main.async() {
                self.parent.viewModel.triggerRefresh()
//                self.parent.triggerRefreshView = true
            }
        }
